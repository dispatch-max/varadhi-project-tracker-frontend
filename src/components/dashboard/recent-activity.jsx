...
//     async function fetchActivity() {
//       try {
//         setActivity(await dashboardApi.getActivity())
//       } catch {
//         setActivity([])
//       } finally {
//         setIsLoading(false)
...
//       {isLoading ? (
//         <div className="flex min-h-0 flex-1 items-center justify-center text-[11px] text-slate-400">Loading...</div>
//       ) : activity.length === 0 ? (
//         <div className="flex min-h-0 flex-1 items-center justify-center text-[11px] text-slate-400">No recent activity yet.</div>
//       ) : (
//         <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
//           <div className="divide-y divide-slate-100">
//             {activity.map((item) => (
//               <div key={item.id} className="flex items-start gap-3 px-4 py-2.5 hover:bg-slate-50">
//                 <div className={cn('mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold text-white', getAvatarColor(item.user.name))}>
//                   {getInitials(item.user.name)}
//                 </div>
//                 <div className="min-w-0 flex-1">
//                   <p className="line-clamp-2 text-[11px] leading-[1.35] text-slate-600">
//                     <span className="font-semibold text-slate-800">{item.user.name}</span>{' '}
//                     {item.message}
//                   </p>
//                   <RelativeTime date={item.createdAt} className="mt-1 block text-[9px] text-slate-400" />
//                 </div>
//               </div>
//             ))}
//           </div>
