...
function Shell({ children }) {
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-100/80 via-purple-50/60 to-blue-100/70 shadow-sm">
      <div className="flex h-[44px] shrink-0 items-center justify-between border-b border-white/50 px-4">
...
export function NotificationsCard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const { notifications } = await notificationsApi.list({ limit: 5, unreadOnly: true })
  ...
  if (isLoading) return <Shell><div className="flex flex-1 items-center justify-center text-[11px] text-slate-400">Loading...</div></Shell>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3">
        <div className="space-y-2">
          {items.map((item) => {
            ...
            const body = (
              <div className="flex items-start gap-2.5 rounded-xl border border-white/60 bg-white/75 px-3 py-2.5">
                <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${style.dot}`} />
                <p className="line-clamp-2 text-[10px] leading-[1.35] text-slate-700">
                  <span className={`font-semibold ${style.text}`}>{style.label}:</span>{' '}
                  {item.title || item.message}
                </p>
              </div>
            )
            return item.linkTo ? <Link key={item.id} href={item.linkTo}>{body}</Link> : <div key={item.id}>{body}</div>
          })}
        </div>
      </div>
    </Shell>
